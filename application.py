...
def getSeverityDict():
    ...
    with open('Symptom_severity.csv') as csv_file:
        ...
        for row in csv_reader:
            try:
                # Attempt to convert the severity value to an integer
                severityDictionary[row[0]] = int(row[1])
            except ValueError:
                # Handle the error if conversion fails (e.g., non-integer value)
                print(f"Warning: Could not convert severity value for symptom '{row[0]}' to an integer. Skipping this entry.")
            except IndexError:
                # Handle the error if the row doesn't have enough columns
                print(f"Warning: Missing severity value for symptom '{row[0]}'. Skipping this entry.")
